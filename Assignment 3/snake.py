...
def check_for_events(direction):
...
def update_snake(direction,snake_pos,snake_body,food_pos,food_spawn,score):
    ...
    if rect1.colliderect(rect2) :
        ...
        game_over(game_window,frame_size_x)
...
def create_food(food_spawn,food_pos):
...
def show_score(pos, color, game_window):
...
def update_screen(game_window,snake_body,food_pos):
    game_window.fill(bg_color)
    for i in range(len(snake_body)):
        pygame.draw.rect(game_window,snake_color,pygame.Rect(snake_body[i][0], snake_body[i][1], 10, 10))
    pygame.draw.rect(game_window,food_color,pygame.Rect(food_pos[0], food_pos[1], 10, 10))
    show_score([10,10],(255,255,0), game_window)
    pygame.display.flip()

def game_over(game_window,frame_size_x):
    ...
    show_score([10,10], (20,34,190), game_window)
    pygame.display.flip()
    time.sleep(3)
    sys.exit(0)
...
# Main loop
while True:
    check_for_events(direction)
    
    update_snake(direction,snake_pos,snake_body,food_pos,food_spawn,score)
    
    create_food(food_spawn,food_pos)

    update_screen(game_window,snake_body,food_pos)
